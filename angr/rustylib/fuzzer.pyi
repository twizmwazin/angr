...
class Fuzzer:
    def __init__(
        ...
        engine: SuccessorsEngine | Callable[[Project], SuccessorsEngine] | None = None,
    ):
        """
        Initialize the fuzzer with the given parameters.

        :param engine: The engine used to execute each input, given either as an engine or as a
            callable building one from the project (an engine class qualifies). It is built at
            most once and reused across executions, and is responsible for maintaining the
            state's ``edge_hitmap`` plugin, which the fuzzer reads back as coverage feedback.
            Defaults to :class:`angr.engines.icicle.UberIcicleEngine`.
        """
