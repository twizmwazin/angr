...
from angr.ailment.expression import BinaryOp, Call, Const, Expression, Load, Phi, VirtualVariable
from angr.ailment.statement import Assignment, ConditionalJump, Label
...
class InlinedStrlenSimplifier(OptimizationPass):
    ...
    def _convert_block_to_strlen(
        ...
        strlen_call = Call(
            self.manager.next_atom(), func_name, args=[str_expr], bits=result_var.bits, ins_addr=block.addr
        )
        strlen_stmt = Assignment(self.manager.next_atom(), result_var, strlen_call, ins_addr=block.addr)
