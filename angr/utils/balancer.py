...
class Balancer:
    ...
    def _unpack_truisms_not(c: Bool) -> set[Bool]:
        ...
        if arg.op == "And":
            return Balancer._unpack_truisms(claripy.Or(*[claripy.Not(a) for a in cast(tuple[Bool, ...], arg.args)]))
        if arg.op == "Or":
            return Balancer._unpack_truisms(claripy.And(*[claripy.Not(a) for a in cast(tuple[Bool, ...], arg.args)]))
        return set()
    ...
    def _balance_sub(truism: Bool) -> Bool:
        ...
        other_adds = cast(tuple[BV, ...], old_lhs.args)[1:]
