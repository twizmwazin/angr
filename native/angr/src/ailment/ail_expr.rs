...
impl AilExpression {
    /// Render this expression to its ``str()`` form.
    ///
    /// Takes `&self` rather than going through the pyclass wrapper: the
    /// wrapper is only needed to hand an expression to Python, and
    /// building one here deep-cloned the subtree and re-entered the
    /// interpreter once per node purely to throw the wrapper away.
        ...
        match &self.inner {
            ...
            ExprInner::ComboRegister { registers, .. } => {
                let parts: Vec<String> = registers
                    .iter()
                    .map(|r| r.render(py).unwrap_or_default())
                    .collect();
                ...
                let args_str = match args {
                    ...
                    Some(v) => {
                        let parts: Vec<String> =
                            v.iter().map(|x| x.render(py).unwrap_or_default()).collect();
                        format!("({})", parts.join(", "))
                    }
                };
            ...
            ExprInner::DirtyExpression {
                ...
                let parts: Vec<String> = operands
                    .iter()
                    .map(|o| o.render(py).unwrap_or_default())
                    .collect();
